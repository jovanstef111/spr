...
% finding vector xi needed for reduced-order interpolant
      ...
      xi6=null((-s6'*eye(ell)+hApi')*XX);
      norm( (Cminii-Cplii)*xi6 ) - norm( (Cminii+Cplii)*xi6 )

      if ii~=i+1
      pom1=(Cminii-Cplii)*[xi5,xi6];
      pom2=(Cminii+Cplii)*[xi5,xi6];
      else
      pom1=(Cminii-Cplii)*[xi1];
      pom2=(Cminii+Cplii)*[xi1];
      end
      U=real(pom1*pinv(pom2));
      % U=[0.7 0.01; 0.03 0.2]; % checking formula for interpolant with U
      norm(U)
...
(DH+CH*(s6*eye(eH)-AH)^(-1)*BH)*pom2(:,6)-pom1(:,6)
...
(DH+CH*(s6*EH-AH)^(-1)*BH)*Cmin(:,6)-Cpl(:,6)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
